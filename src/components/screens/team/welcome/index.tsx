...
export default function TeamWelcome({ teamId }: Props) {
    const teamName = teamId === "a" ? "TEAM (A)" : "TEAM (B)";
    return (
        <motion.section
            className="w-full h-screen relative flex items-end justify-center pb-30 overflow-hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
